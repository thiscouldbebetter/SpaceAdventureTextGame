			...
			[
				...
				(
					...
					(
						...
						this.scripts.regionFriendlyShip_HideUnsuccessfully.name
					)
				),
			]
		);
...
class Scripts
{
	...
	): void
	{
		...
		w.agentPlayer.stateWithNameSetToValue
		(
			stateName, 0
		);
	...
	regionPlanetDesert_UpdateForTurn(u: Universe, w: World, p: Place, c: Command): void
	{
		...
		var turnsSinceArriving =
			playerState.stateWithNameGetValue(stateNameTurnsSinceArriving);
		...
		if (turnsSinceArriving == 15)
		{
			...
			var agentAssassinDrone = Agent.fromNames
			(
				[ "drone", "assassin drone", "robot", "spider robot",  ]
			).descriptionAsPartOfPlaceSet
			(
				[
			...
			(
				Script.fromName
				(
					Scripts.Instance().regionPlanetDesert_AssassinDroneUpdate.name
				)
			);
		...
		turnsSinceArriving++;
		playerState.stateWithNameSetToValue
		(
			stateNameTurnsSinceArriving, turnsSinceArriving
		);

		var stateNameTurnsSinceLastDrink = "TurnsSinceLastDrink";
		var turnsSinceLastDrink =
			playerState.stateWithNameGetValue(stateNameTurnsSinceLastDrink);
		...
		playerState.stateWithNameSetToValue
		(
			stateNameTurnsSinceLastDrink, turnsSinceLastDrink
		);
	}

	regionFriendlyShip_HideUnsuccessfully(u: Universe, w: World, p: Place, c: Command): void
	{
